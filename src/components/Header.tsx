...
import { enterFullscreen, exitFullscreen, formatTimeElapsed, getTodaysId, isFullscreen, randU32 } from "../funcs";
import { createSideEffect, selectCompletedBoards, showPopup, startGame, useAppDispatch, useSelector } from "../store";
...
function Row1() {
  ...
  const handleHistClick = () => {
    ...
    const res = window.prompt("Play a Historical Duotrigordle!\n" + "Enter a past Daily Duotrigordle # to play:");
    if (res === null) return;
  ...
  return (
    <div className="row-1">
      {practice ? (
        <>
          <button className="mode-switch" ref={backRef} onClick={handleBackClick}>
            Back
          </button>
          <button className="mode-switch" ref={histRef} onClick={handleHistClick}>
            Hist
          </button>
        ...
        <>
          <button className="mode-switch" ref={practiceRef} onClick={handlePracticeClick}>
            Practice
          </button>
          <div />
          <div />
      ...
      <button className="icon" onClick={() => dispatch(showPopup("stats"))}>
        <Stats role="img" aria-label="Stats"></Stats>
      </button>
      ...
      <button className="icon" onClick={handleFullscreenClick}>
        <img src={fullscreen ? fullscreenExitSvg : fullscreenSvg} alt="Go Fullscreen" />
      </button>
    </div>
  );
}
...
function Row2() {
  ...
  const boardsCompleted = useMemo(
    () => targets.map(target => guesses.includes(target)).reduce((a, v) => a + (v ? 1 : 0), 0),
    [targets, guesses],
  );
  const numGuesses = guesses.length;
  const gameOver = useSelector(s => s.game.gameOver);
  const extraGuessesNum = NUM_GUESSES - NUM_BOARDS - (numGuesses - boardsCompleted);
  const cannotWin = extraGuessesNum < 0;
  const extraGuesses = extraGuessesNum > 0 ? "+" + extraGuessesNum : extraGuessesNum;
  ...
  useEffect(() => {
    const handler = (k: KeyboardEvent) => {
      ...
      if (
        ...
        !k.shiftKey &&
        !k.altKey &&
        ((useCmdKey && !k.ctrlKey && k.metaKey) || (!useCmdKey && k.ctrlKey && !k.metaKey))
      ) {
        k.preventDefault();
        dispatch(startGame({ id: randU32(), practice: true }));
        setReset(true);
  ...
  return (
    <div className="row-3">
      {boardsCompleted.map((complete, i) => (
        <button
          key={i}
          className={cn("chip", complete && "complete", !complete && highlightedBoard === i && "highlight")}
          onClick={() =>
            dispatch(
              createSideEffect({
